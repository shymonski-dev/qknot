...
Submits build_sl3_hadamard_circuit to IBM hardware via the
/api/knot/sl3/submit and /api/knot/sl3/poll routes, then prints
the result including hadamard_expectation and classical_reference.

Required environment variables:
    QKNOT_BACKEND_NAME   IBM backend to target (e.g. ibm_fez)

Optional environment variables:
    QKNOT_BACKEND_URL          Backend API base URL (default: http://127.0.0.1:8000)
    QKNOT_BRAID_WORD           Braid word to evaluate (default: s1 s2 s1 s2, trefoil)
    QKNOT_SHOTS                Number of shots (default: 512)
...
def main() -> int:
    ...
    braid_word = _read_optional_env("QKNOT_BRAID_WORD") or "s1 s2 s1 s2"
    shots = int(_read_optional_env("QKNOT_SHOTS") or "512")
    ...
    print(f"Submitting sl_3 Hadamard circuit smoke job...")
    ...
    for attempt in range(1, max_poll_attempts + 1):
        ...
        if status == "COMPLETED":
            ...
            print(f"  deviation            : {poll_response.get('deviation')}")
